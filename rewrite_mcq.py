...
# We need to change the QTemplate helper class in RaisecService.java first,
# because it was storing text, now it needs to store prompt, List<String> options, and answerIndex.
# It's actually easier to just use RaisecQuestion directly.
...
java_code = """
        ...
        private static final List<QTemplate> bank = new ArrayList<>();
        static {
            int c = 1;
            // ==========================================
...
replacement_pattern = re.compile(r"public static class QTemplate \{.*?\}\s*\}", re.DOTALL)
new_content = replacement_pattern.sub(java_code, content)

# Now edit the convert method:
# private RaisecQuestion convert(QTemplate t) {
#    return RaisecQuestion.builder().id(t.id).prompt(t.prompt).options(t.options).answerIndex(t.answerIndex).raisecLetter(t.letter).difficulty(t.diff).stream(t.stream).build();
# }

convert_pattern = re.compile(r"private RaisecQuestion convert\(QTemplate t\) \{.*?\}", re.DOTALL)
new_convert_method = """private RaisecQuestion convert(QTemplate t) {
        return RaisecQuestion.builder()
                .id(t.id)
                .prompt(t.prompt)
                .options(t.options)
                .answerIndex(t.answerIndex)
                .raisecLetter(t.letter)
                .difficulty(t.diff)
                .stream(t.stream)
                .build();
    }"""
new_content = convert_pattern.sub(new_convert_method, new_content)
